// Setup
// Codepen: https://codepen.io/drboolean/pen/zYYPmZO?editors=0010
const _ = R;
...
var averageDollarValue_ = function(cars) {
  var dollar_values = _.map(function(c) { return c.dollar_value; }, cars);
  return _average(dollar_values);
};

// We need to map over the entire Cars array collecting the value of "dollar_value" for each car and we pass that to average
...
const availablePrices__ = _.compose(
  _.join(', '),
...
// We first need to grab the "horsepower" property then sort them
// grab the last element because it would be the highest horsepower
// grab the name of this last element
// concatonate the extra string
// flip it so the sentance makes sense
