...
Alternative patterns involve binding an existing function in the constructor, or assigning the function in the constructor.
...
> **Avoid** using `bind` to bind a function to its outer `this`
> Instead use arrow functions, as they make `this` clear from the function declaration, rather than something that happens later in the code
> 
...
> Consider passing the value as an argument instead; it's more explicit, and works with arrow functions.
...
> Don't use call/apply to set this to some value unrelated to the parent object; it's usually unexpected and it's why this gets such a bad reputation. 
> Consider passing the value as an argument instead; it's more explicit, and works with arrow functions
...
#### If the function is called via a parent object (parent.func()):
...
In this case the function is called as a member of `obj`, so `this` will be `obj`.
...
`someMethod() === obj` is false because `someMethod` isn't called as a member of `obj`.
...
#### If function or parents scope is in `strict mode`
...
In this case, the value of this is undefined.
`'use strict'` isn't needed in the function if the parent scope is in strict mode (and all modules are in strict mode).
...
<p align="center">
  ![ImplicitBinding](/Images/implicitBinding2.png)
</p>
